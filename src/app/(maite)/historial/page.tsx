...
export default async function HistorialPage({
  ...
  const summary = await getWeekSummary(session.sub, weekStart);
  ...
  return (
    <div className="mx-auto max-w-md space-y-5">
      <div className="flex items-center justify-between">
        <Link
          href={`/historial?week=${prevWeekKey}`}
        ...
        {isCurrentWeek ? (
          ...
          <Link
            href={`/historial?week=${nextWeekKey}`}
            className="rounded-lg px-3 py-2 text-sm font-medium text-slate-500 active:bg-slate-100"
          >
            Siguiente →
          </Link>
      ...
      <section className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <p
              className={`text-xl font-bold ${summary.deficitAcumulado >= 0 ? "text-green-700" : "text-red-600"}`}
            >
