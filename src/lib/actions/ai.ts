...
import { analyzeFoodPhoto, analyzeNutritionLabel } from "@/lib/ai/food-photo";
...
export async function analyzeNutritionLabelAction(input: z.infer<typeof analyzeSchema>) {
  await requireUsuaria();
  const { base64Image, mediaType } = analyzeSchema.parse(input);
  const result = await analyzeNutritionLabel(base64Image, mediaType);
  if (!result.reconocido) {
    throw new Error(
      "No se ha reconocido una etiqueta de información nutricional en la foto. Prueba con otra imagen o rellena los datos a mano.",
    );
  }
  return result;
}
